...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        Long issuedTime = Long.parseLong(JwtUtil.getClaim(token,"currentTimeMillis"));
        Long expireTime = Long.parseLong(Properties.getRefreshTokenExpireTime()) * 1000;
        // 说明尚未过期
        if(System.currentTimeMillis()<issuedTime+expireTime){
            BackendUser user = BackendUserService.getInstance().selectByUserName(username);
            // 获取当前最新时间戳
    ...
    private void response401(ServletRequest req, ServletResponse resp, String msg) {
        ...
        try {
            ...
            out.append("无权访问(Unauthorized):" + msg);
        } catch (IOException e) {
